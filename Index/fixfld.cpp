...
static const char * fldheaderStr = "Field";
static const int fldheaderStrSize = strlen (fldheaderStr);
...
{
	char * str = new char[fldheaderStrSize+1];
	...
	stream . read (str, fldheaderStrSize);
	if (!stream.good()) return -1;
	if (strncmp (str, fldheaderStr, fldheaderStrSize) != 0) return -1;
...
//	field sizes			4 bytes per field
{
	...
	stream . write (fldheaderStr, fldheaderStrSize);
	if (!stream.good()) return -1;
