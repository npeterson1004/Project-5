...
const char * varheaderStr = "Variable";
const int varheaderSize = strlen (varheaderStr);
...
// read the header and check for consistency
{
	...
	if (strncmp (str, varheaderStr, varheaderSize) != 0) return FALSE;
...
//		that describes the file records
{
	...
	stream . write (varheaderStr, headerSize);
	if (!stream . good ()) return FALSE;
