...
static const char * lenheaderStr = "Fixed";
static const int lenheaderStrSize = strlen (lenheaderStr);
...
// see WriteHeader for header record structure
{
	...
	stream . read (str, lenheaderStrSize);
	if (!stream.good()) return -1;
	if (strncmp (str, lenheaderStr, lenheaderStrSize) != 0) return -1;
...
//	record size		2 bytes
{
	...
	stream . write (lenheaderStr, lenheaderStrSize);
	if (!stream . good ()) return -1;
